<div class="w-full h-full">
    ...
    <div class="w-full h-full flex flex-col pt-24 p-8 gap-8">
            ...
            <div class="flex flex-col gap-4 w-full h-full">
                ...
                <div class="bg-aqua border-2 border-black rounded-2xl">
                    ...
                    <div class="p-4 flex flex-col gap-2 pb-8">
                        ...
                        <select wire:model.live="jenisKendaraan" class="p-2 bg-abswhite rounded border border-black">
                ...
                @foreach ($penumpang as $index => $p)
                    <div wire:key="penumpang-wrapper-{{ $jenisKendaraan }}" class="bg-aqua border-2 border-black rounded-2xl">
                        <div class="text-2xl max-md:text-lg text-black p-4 border-b-2 border-black">Identitas Penumpang {{ $index + 1 }}</div>
                        <div class="p-4 flex flex-col gap-2 pb-8">
                            <p class="text-lg text-black">Nama Penumpang</p>
                            <input type="text" wire:model.live="penumpang.{{ $index }}.nama" class="p-2 bg-abswhite rounded border border-black" placeholder="Nama Penumpang">
                            <p class="text-lg text-black">Nomor Identitas</p>
                            <input type="text" wire:model.live="penumpang.{{ $index }}.identitas" class="p-2 bg-abswhite rounded border border-black" placeholder="0123456789">
                        </div>
                    </div>
                @endforeach
                <button 
                    type="button" 
                    wire:click="tambahPenumpang" @disabled(count($penumpang) >= $kapasitasKendaraan[$jenisKendaraan]) 
                    class="flex items-center justify-center gap-4 bg-yellow hover:bg-[#FFA000] text-white rounded-lg px-6 py-3 font-bold active:scale-90 transition duration-200 disabled:bg-gray-400 disabled:cursor-not-allowed disabled:active:scale-100"
                >
        ...
        <button href="{{ route('etiket') }}" wire:navigate class="p-4 text-xl w-60 rounded-full bg-orange border-dark text-white font-bold active:scale-90 duration-100 transition-all border-2 mx-auto">
            Pesan
        </button>
    </div>
